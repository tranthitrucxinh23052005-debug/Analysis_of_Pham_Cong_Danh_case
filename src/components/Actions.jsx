...
export default function Actions() {
  ...
  return (
    <section className="py-20 px-8 bg-white/20">
      <div className="max-w-4xl mx-auto">
        <motion.h2 
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          className="text-3xl md:text-4xl font-bold text-[#F075AE] mb-10 text-center drop-shadow-[0_0_5px_#FFAAB8]"
        >
        ...
        <div className="space-y-4">
          {actions.map((action, index) => (
            <div key={index} className="border-b border-[#FFAAB8]/30 pb-4">
              <button 
                onClick={() => setActiveIndex(activeIndex === index ? null : index)}
                className="w-full text-left p-4 bg-[#FFD8DF] rounded-xl hover:bg-[#FFAAB8] transition flex justify-between items-center group"
              >
                <span className="font-bold text-[#1a1a1a] flex items-center gap-3">
              ...
              <AnimatePresence>
                {activeIndex === index && (
                  ...
                  >
                    <p className="p-4 text-[#555] leading-relaxed italic bg-white/30 mt-2 rounded-lg border-l-4 border-[#F075AE]">
                      {action.desc}
                    </p>
                  </motion.div>
                )}
